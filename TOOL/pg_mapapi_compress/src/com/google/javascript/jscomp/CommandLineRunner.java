...
public class CommandLineRunner extends
  ...
  public CommandLineRunner(String[] args, PrintStream out) {
    super(out);
    initConfigFromFlags(args, System.err);
  }
   ...
   */
  public static void main(String[] args) {
	  //--compilation_level SIMPLE_OPTIMIZATIONS --js js/api.js --js_output_file js/api.js

	 String[] args3={
	 		 "--compilation_level","SIMPLE_OPTIMIZATIONS",
	 		 "--js","E:/jsencrypt/js11/ObscureJS_1.1.js",
	 		 "--js_output_file","E:/jsencrypt/js11/ObscureJS_google_1.1.js"
	 		// "--js","E:/JAVA_WorkSpace/se_javascriptapi3_taiwan/SRC/PUBLIC/tw.smartearth.cn/all/all.js",
		 	 //"--js_output_file","E:/JAVA_WorkSpace/se_javascriptapi3_taiwan/SRC/PUBLIC/tw.smartearth.cn/js/api1.js"
	 		};
    CommandLineRunner runner = new CommandLineRunner(args3);
   // String a = "var fdsgfds = function(p){var a=0,s=0;var d=p.length;var f=new String();var g=-1;var h=0;for(var j=0;j<d;j++){var k=p.charCodeAt(j);k=(k==95)?63:((k==44)?62:((k>=97)?(k-61):((k>=65)?(k-55):(k-48))));s=(s<<6)+k;a+=6;while(a>=8){var l=s>>(a-8);if(h>0){g=(g<<6)+(l&(0x3f));h--;if(h==0){f+=String.fromCharCode(g);};}else{if(l>=224){g=l&(0xf);h=2;}else if(l>=128){g=l&(0x1f);h=1;}else{f+=String.fromCharCode(l);};};s=s-(l<<(a-8));a-=8;};};return f;};";
   // runner.setPGSource(a);    
    if (runner.shouldRunCompiler()) {
      runner.run();
    } else {
      System.exit(-1);
